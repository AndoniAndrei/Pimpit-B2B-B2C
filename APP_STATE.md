...
- 2026-06-03 — **Honesty audit: eliminat datele fabricate din UI**: după feedback („absolut toate informațiile din site trebuie să fie reale, ex: nu afișa KBA dacă nu există în DB"). Eliminate: **(1) Cert badges KBA + JWL hardcoded** pe product page (`app/jante/[slug]/page.tsx`) — acum doar TÜV apare, condiționat de `product.certificate_url`. **(2) PromoBanner** rescris — eliminat "30% OFF" inventat și subtitlul "Hyper & luxury — Concaver, JUDD, OZ" (brand-uri specifice). Acum 3 plăci pur navigative pe baza diametrului (17-18, 19-20, 21+) cu descrieri generice de segment. **(3) "Livrare & retururi" accordion** pe product page — eliminat ("24-48h", "1500 RON", "14 zile" — toate claim-uri nesusținute). Comentariu cu re-activare după ce există date reale. **(4) "Verifică compatibilitatea cu mașina ta" accordion** — eliminat. Verificarea de fitment necesită tabelă vehicle ⇄ spec care nu există. `VehicleSelector` importul eliminat din product page. **(5) VehicleSelector copy** — schimbat headline din "Selectează vehiculul / Găsim jantele compatibile" → "Mașina ta / Selecția vehiculului nu filtrează încă fitment-ul". CTA „Caută jante compatibile" → „Răsfoiește catalogul". **(6) NewsletterSignup** — șters complet (componenta + Footer reference). Form-ul nu avea backend wiring — submit fabrica un success state. Re-introduceere doar când se conectează la tabel real. **(7) CategoryTiles** — eliminat 3 plăci cu filtre care nu există în DB: „Performanță" (`?category=performance`), „Stilizare" (`?finish=Lustruit`), „Sezoniere" (fără filtru sezonier). Înlocuit cu „Cele mai noi", „Preț ↑", „Toate brand-urile". Eliminat sublabels editoriale. **(8) Footer brand list** — eliminate Concaver/Japan Racing/JUDD/Sixnine/MB Design hardcoded (poate nu există în DB). Înlocuit cu listă „Răsfoiește" — link-uri generice. **(9) Trust strip** (Footer) — „Certificat TÜV / KBA" și „Livrare 24-48h în țară" eliminate. Înlocuit cu „Catalog aftermarket", „Livrare națională", „Garanție producător", „Plată securizată" (generice, fără cifre). **(10) SocialProof** — „24-48h"/"TÜV / KBA" înlocuite cu „Curier"/"Plată sigură"; productCount și brandCount rămân (vin din DB real). **(11) FeaturedBrands subtitle** — „Producători aftermarket cu certificare TÜV / KBA" → „Producători aftermarket disponibili acum". Titlu „Brand-uri populare" → „Brand-uri în catalog". **(12) TrendingProducts subtitle** — „Cele mai bine vândute jante din ultima săptămână" (sales velocity netracked) → „Disponibile din stoc / Jante cu cea mai mare disponibilitate în acest moment". Titlu „Cele mai căutate" → „Disponibile din stoc". **(13) Hero subtitle** — „Catalog unificat · mii de modele aftermarket · fitment garantat" → „Caută după diametru, lățime, PCD, ET și CB — filtrele catalogului mapează exact fitment-ul jantei". **(14) Footer brand tagline** — „Aftermarket verificat, fitment exact, livrare rapidă" → „Catalog ... agregate din mai mulți furnizori aftermarket". **(15) Metadata description** — „Cel mai mare catalog de jante auto din România" (superlative) → „Catalog de jante auto aftermarket agregat din mai mulți furnizori". Product page metadata „la cel mai bun preț" → "disponibilă pe pimpit.ro". **(16) Navbar search placeholder** — exemplu „19x9.5 5x112" eliminat (search-ul caută în name/brand/part_number, nu în dimensiuni). Build curat. — strict no-fabrication policy aplicată
- 2026-06-03 — **Native system font + pure black + shimmer gold**: la feedback repetat (Inter încă nu place). **Fonturi:** drop Inter complet, switch la **stack native** (`-apple-system, BlinkMacSystemFont, "Segoe UI Variable", "Segoe UI", Roboto, "Helvetica Neue", Arial`). Layout.tsx — eliminat next/font Inter, body className `font-sans`. Body cu `-webkit-font-smoothing: antialiased` + `text-rendering: optimizeLegibility`. Tailwind sans/display ambele → system stack. JetBrains Mono rămâne doar pe spec chips. **Culori:** `pimpit.text` → **#000000 (pur negru)**, `text-muted` → #404040 (zinc-700), `accent` → #A8841D (rich saturated gold, 5.2:1 contrast), `accent-hover` → #876715, `accent-light` → #F5D06B (bright shine highlight). Shadcn `--foreground: 0 0% 0%`. **Gold buttons rescriși în globals.css** — brighter metallic gradient (F5D06B → D4AF37 → B8860B → 876715), inset white highlight + dark inset shadow (embossed look), drop shadow gold layered 2px+6px, **continuous shimmer animation** (`gold-shimmer 6s ease-in-out infinite` care drift-uie background-position pe gold metallic), white text cu text-shadow subtil 1px. Hover: gradient mai luminos + 1px lift + shadow mărit. `.text-gold-shine` cu animație 5s linear infinite pentru logo .RO și highlight words. `btn-gold-outline` cu fill gradient gold pe hover. Build curat. — definitive premium typography
- 2026-06-03 — **Premium polish: promo banner + newsletter**: continuare a overhaul-ului premium. **PromoBanner** (`components/home/PromoBanner.tsx`) — strip cu 3 plăci editoriale large între Hero și SocialProof, fiecare cu eyebrow + headline mare + sub + CTA, decoreted cu CSS wheel ring în colț. 3 variante de accent (red / gold / dark) cu gradient + shine wash specific. Linkează la /jante cu filtre (oferte preț crescător / diametru 20"+ / diametru 18"). **NewsletterSignup** (`components/shared/NewsletterSignup.tsx`) — strip premium înainte de footer cu icon Mail rotund auriu, headline „Acces la oferte premium", input email + buton btn-gold „Abonează-mă", optimistic UX (CheckCircle pe success). Backend (Mailchimp/Brevo/Supabase newsletter table) — TODO. Adăugat în `Footer.tsx` ca primă secțiune. Build curat. — premium polish enhancement
- 2026-06-03 — **Premium light overhaul (Inter + shiny gold)**: la feedback („nu îmi place fontul, vreau ceva simplu și lizibil, gold cu shine pe alb, totul să strige PREMIUM"). **Fonturi:** drop Barlow Condensed; Inter (weights 300-900) e singurul font de UI, `font-display` aliased to Inter în `tailwind.config.ts` ca să nu trebuiască să rescriu fiecare componentă. JetBrains Mono rămâne doar pe chip-urile fitment (Ø/J/PCD/ET/CB). **Culori:** `pimpit.text` → #0A0A0A (near-black pentru max readability), `text-muted` → #525252 (zinc-600), `accent` → #B8860B (DarkGoldenRod, 4.74:1 contrast pe alb), `accent-hover` → #9A6F08, nou `accent-light` → #E5B95A pentru highlight-ul din gradient. **Gold shine:** utility class `.btn-gold` în `globals.css` cu gradient metallic (E5B95A → C9A227 → B8860B → 8B6914), shadow gold soft, și un pseudo-element ::before care animează un wash diagonal alb pe hover (sweep stâng→drept 0.7s). Variant outline `.btn-gold-outline` cu 2px border, fill transparent. Gold text gradient utility `.text-gold-shine` pentru logo .RO și highlight-ul din hero. **Hero rescris:** layout split CARiD-style — card vehicle selector pe stânga (360px, alb cu shadow-premium, 3 dropdown-uri numerotate 1/2/3 + buton gold-shine), promo banner pe dreapta (aspect 16/7+, fundal dark cu wheel CSS pe partea dreaptă, headline cu „mașina ta" în gold-shine gradient text, CTA gold cu shine). **Navbar:** restructurat în 2 rânduri (CARiD pattern) — rândul 1: logo + search bar prominent (cu icon de submit pe dreapta) + cont/coș. Rândul 2 (desktop): nav Jante/Accesorii. Padding main pentru desktop bumped la pt-[6.75rem] (108px). **ProductCard:** rescris pe Inter natural (drop uppercase tracking dur), CTA pe `.btn-gold`, badge discount -X% pe stânga sus, chip culoare pe dreapta sus, specs cu fitment chips, hover shadow-premium-hover. **Homepage sections:** CategoryTiles redesign cu 10 plăci icon-based (Disc/Wrench/Settings/Gauge/Layers/etc.), fiecare cu cerc icon + label + sub. FeaturedBrands ca chip-uri (5 coloane), SocialProof cu icons rotund auriu, TrendingProducts cleaned. SectionHeading helper simplu (titlu bold + subtitlu muted). **Catalog:** FilterSidebar cu Inter natural, checkbox-uri custom gold-fill, pill buttons cu shadow-gold când active, accordion mai compact. MobileFilters cu rounded-t-lg, CTA pe `.btn-gold`. CatalogControls cu titlu mai mare bold. Pages `/jante` și `/accesorii` cu fundal `bg-pimpit-surface-2` (warm wash), sidebar și header bar `bg-white` ca insule cu shadow-premium. Sticky header top mutat la `lg:top-[6.75rem]` pentru navbar 2-row. **Product page:** restilizat — info card pe dreapta cu `bg-white rounded-md shadow-premium p-7`, accordion separat cu fundal alb dedesubt. Drop monospace din SpecRow (acum text muted + semibold dark). CertBadge cleaner. CTA primary cu `.btn-gold`, secondary cu `.btn-gold-outline` (cumpără acum). Related ca scroll orizontal cu carduri 280-300px. **Tokens shadcn:** primary HSL `36 90% 38%` (B8860B), foreground HSL `0 0% 4%` (near-black), muted-foreground `0 0% 32%` (zinc-600), border `30 8% 90%`. **shadow tokens:** premium / premium-hover / gold în tailwind.config pentru depth fără să cazi în material-design. Build curat pe toate 38 rute. — premium light system
- 2026-06-03 — **REVERT dark mode → light theme**: user a anulat complet dark mode. `app/globals.css` `:root` și `.dark` flipped înapoi la valori light (`background` white, `foreground` stone-900, `primary` deep gold 44 54% 48%, `border` stone-200). `color-scheme: light`. `tailwind.config.ts` namespace `pimpit.*` re-mapat la valori light: `bg` #FFFFFF, `surface-2` #FAFAF9, `border` #E7E5E4, `accent` #B5933A (deeper gold pentru contrast pe alb), `text` #1C1917, `text-muted` #78716C, `success` green-600, `error` red-600. Toate componentele continuă să folosească aceleași `pimpit-*` clase fără rescriere. Patch-uri inline: `Hero.tsx` cu gradient light + wheel CSS rămâne dark ca element editorial (intenționat — render produs pe pagină albă). `ProductCard.tsx` image gradient → from-stone-50 via-white to-stone-100, hover spec overlay → white/95. Toate referințele `text-pimpit-bg` (care acum era alb pe gold = contrast fail 3.0:1) înlocuite global cu `text-stone-900` per spec-ul original „text-black pe gold". Build curat. — anulare dark mode
- 2026-06-03 — **CARiD-style overhaul + dark global** *(reverted same day → light)*: aplicat structura UI/UX CARiD pe paleta pimpit dark+gold. **(1) Tokens** — `tailwind.config.ts` extins cu namespace `pimpit.*` (bg/surface/surface-2/border/accent/accent-hover/text/text-muted/success/error). `app/layout.tsx` înlocuit Space Grotesk → **Barlow Condensed** (`--font-display`, weights 400-800), păstrat JetBrains Mono + Inter (`--font-body`). `app/globals.css` — `:root` și `.dark` flipped la pimpit dark (background `#0A0A0A`, foreground `#F5F5F5`, primary `#C9A84C` gold, radius `0.25rem`). `color-scheme: dark` setat global. Utilitate `.no-scrollbar`. **(2) Navbar** — `components/shared/Navbar.tsx` rescris: logo gold accent + Jante/Accesorii nav uppercase + search bar prominent (în URL bar, redirect /jante?search=) + cont/coș icons + mobile drawer. **(3) Footer nou** — `components/shared/Footer.tsx`: 4 coloane (brand+tagline, Catalog, Cont, Brand-uri) + trust strip (TÜV/KBA, 24-48h, garanție, plată securizată) + bottom strip cu copyright și metode de plată. Adăugat în `app/layout.tsx`. **(4) Hero homepage** — `components/home/Hero.tsx` cu **VehicleSelector** Year/Make/Model embedded (`components/home/VehicleSelector.tsx` + `lib/vehicleData.ts` cu 26 mărci europene/românești și ~150 modele). Submit → `/jante?vehicle=YYYY-Make-Model` (vehicle-FK lookup e enhancement viitor, nu există FK în schema produse). Păstrat wheel anchor CSS-only + parallax. **(5) Homepage sections** — nou `CategoryTiles.tsx` (6 plăci: 17"/18"/19"/20"/21"+/Accesorii cu diametru-ca-typography), `FeaturedBrands.tsx` (scroll orizontal cu brand wordmarks din producători detectați în catalog), `SocialProof.tsx` (4 stat-cells: produse, brand-uri, livrare, TÜV/KBA), `TrendingProducts.tsx` (top 8 by stock, scroll mobile, grid 4-col desktop). `app/page.tsx` rescris ca server component cu un singur query supabase (40 produse stock-sorted). **(6) ProductCard** — refactorizat pe tokens pimpit-* (eliminat hex inline), CTA `bg-pimpit-accent`, Barlow uppercase pentru nume. **(7) Catalog filtre** — `FilterSidebar.tsx` rescris cu accordion uppercase, checkbox-uri custom gold-check, pill-buttons pentru diametru/lățime/ET, chip-uri active. `CatalogControls.tsx` cu search icon și monospace counter. `MobileFilters.tsx` ca **bottom sheet** pe mobile (`items-end`) cu CTA sticky gold. **(8) Pagini catalog** — `app/jante/page.tsx` + `app/accesorii/page.tsx`: breadcrumb top, sticky header dark, empty-state restilizat, paginare dark. **(9) Pagina produs** — `app/jante/[slug]/page.tsx` cu cert badges (TÜV/KBA/JWL inline icon+label), 4 accordion sections (Descriere, Fitment Checker = VehicleSelector embedded, Specificații complete, Livrare & retururi via `components/catalog/AccordionSection.tsx`), related ca scroll orizontal (8 items). `ProductActions.tsx` cu CTA primar gold + secondary "Cumpără acum" border-gold. **(10) Admin migration** — `app/admin/produse/ProduseClient.tsx` + `components/admin/ImportWizard.tsx`: clasele `bg-white`, `text-gray-*`, `bg-gray-*`, `border-gray-*` înlocuite cu shadcn tokens (`bg-card`, `text-foreground`, `text-muted-foreground`, `bg-muted`, `border-border`) pentru a deveni automat dark. Build verifică curat pe toate cele 38 rute. — overhaul complet UI/UX storefront + admin
