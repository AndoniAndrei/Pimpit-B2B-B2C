...
| `/fitment` | `app/fitment/page.tsx` + `VehicleSelector.tsx` | „Ce se potrivește pe mașina ta?" — selector cascadă Marcă→Model→An→Trim, statistici din setup-uri reale (top dimensiuni jante/anvelope, % fără frecări, % staggered), tabel setup-uri, CTA spre `/jante?diameter=X`. Datele vin din `vehicle_fitments` (galeria de 57k). |
| `/c` + `/c/[slug]` | `app/c/page.tsx`, `app/c/[slug]/page.tsx` | **Catalog V2** — index categorii + pagină generică de categorie condusă de `category_filter_definitions`: fațete cu contoare din `product_variants.attrs`, filtru brand, sort, paginare 24/pg, carduri de familie (preț „de la", nr. variante, stoc). MVP: agregare în server component, cap 500 familii — la scală se mută în RPC. |
| `/p/[slug]` | `app/p/[slug]/page.tsx` + `VariantTable.tsx` | **Catalog V2** — pagina familiei (stil CARiD): galerie din `media_assets`, tabel variante cu specificațiile tipizate (coloane doar pentru atributele folosite), stoc, preț + preț vechi, cantitate și add-to-cart per variantă (`POST /api/cart {variant_id}`). |
| `/cos` | `app/cos/page.tsx` | Coș Zustand. Subtotal, shipping (gratuit > 1000 RON altfel 50 RON). Suportă și variante V2 (normalizate de API la același format). |
...
| `/api/cart` | GET/POST/DELETE | Coș pe `user_id` sau `session_id` (cookie). POST acceptă `product_id` (v1) SAU `variant_id` (v2, exact unul). GET normalizează articolele v2 în forma `item.product.*` ca UI-ul existent să meargă neschimbat. |
| `/api/orders` | POST | Creează order + order_items din cart curent. Validează stock. Șterge coșul la success. Articolele v2 (variant_id): stoc/preț din `product_variants` (B2B aware), snapshot nume familie+sufix+brand. |
...
| `/api/vehicles` | GET | Selector vehicul cascadă: fără parametri = mărci; `?make=` = modele; `+&model=` = ani; `+&year=` = trims. Cache 1h + SWR 24h. |
...
| `/admin/vehicule` | `app/admin/vehicule/page.tsx` + `FitmentImportClient.tsx` | Statistici vehicule/fitmenturi + butoane dry-run/import pentru galeria de 57k fitmenturi (rulează server-side prin `/api/admin/fitment-import`). |
| `/admin/import-v2` | `app/admin/import-v2/page.tsx` + `RunJobButton.tsx` | Hub import v2: profiluri de mapare active + istoric joburi cu statusuri; rulare dry-run per profil (feed URL sau fișier nou). |
| `/admin/import-v2/profiles/nou` | `.../profiles/nou/ProfileWizard.tsx` | Wizard 5 pași pentru non-developeri: furnizor+categorie → sursă (upload/URL cu auto-detect format+delimitator) → mapare coloane cu **preview live** (mapper-ul rulează în browser pe rândurile de probă) → preț & reguli (valută, formulă, auto-create branduri, filtru rânduri) → salvare + dry-run. |
| `/admin/import-v2/jobs/[id]` | `.../jobs/[id]/page.tsx` + `JobActions.tsx` | Raport job: contoare, diff staging (create/update/unchanged/deactivate), erori grupate + tabel. Acțiuni contextuale: dry-run → „Rulează importul real"; staged → Publică/Renunță; published → Rollback. |
...
| `/api/admin/fitment-import` | POST | Import galeria de fitmenturi din `data/fitmentgallery.csv.gz` (inclus în deploy prin `outputFileTracingIncludes`). `{apply:false}` = dry-run. Logica în `lib/import/fitmentGallery.ts` (partajată cu scriptul CLI). `maxDuration = 300s`. |
| `/api/admin/import-v2/context` | GET | Context pentru wizard: categorii + atribute (moștenire rezolvată), branduri, cursuri valutare, furnizori. |
| `/api/admin/import-v2/sample` | POST | Coloane + 20 rânduri de probă: multipart (fișier → salvat ca snapshot re-folosibil în `feed-snapshots/uploads/`) sau JSON `{feed_id}` (fetch cu auth-ul furnizorului). Auto-detect format (extensie) + delimitator CSV. |
...
018_cart_orders_v2_variants.sql        → V2: cart.variant_id + order_items.variant_id (aditiv; CHECK product_id SAU variant_id; unique (user|session, variant))
019_publish_price_b2b.sql              → V2: publish_import_job() scrie și price_b2b din offer_payload (înlocuiește funcția din 017)
020_fitments_private_read.sql          → vehicle_fitments: citire doar admin/service role (URL-urile sursă nu mai sunt expuse prin API-ul public)
```
...
- 2026-07-07 — **Protecție anti-confuzie generații (ex. „530i" ≠ o singură prindere)**: analiză pe datele reale — 185/953 modele acoperă ≥10 ani (multi-generație). Nou `lib/fitment/vehicleGenerations.ts`: dicționar șasiuri (BMW complet, VW, Audi, Honda, Subaru, Ford, Toyota, Mazda, Nissan, Mitsubishi, MINI, Lexus, Acura, Chevrolet, Mercedes) cu interval de ani + PCD/alezaj (setate DOAR când sunt uniforme; cazurile mixte au notă, ex. Civic Si EP3, Accord V6). `resolveGeneration(marcă, model, an)` folosit în `/fitment`: badge „Generația: E60/E61 (2004–2010) · Prindere 5x120 · Alezaj 72.6mm", note de avertizare pentru excepții, PCD-ul generației injectat în linkurile spre catalog, banner de avertisment când lipsește anul și rezultatele acoperă ≥8 ani. +7 teste (45 total). — siguranța potrivirii jantelor
- 2026-07-07 — **Fără referințe la sursa fitmenturilor pe frontend**: eliminate mențiunile „Fitment Industries" din UI (admin) și comentariile de cod; migrație `020_fitments_private_read.sql` — `vehicle_fitments` nu mai e citibilă public prin API-ul Supabase (paginile publice citesc server-side cu service role; URL-urile sursă nu se afișau oricum nicăieri). Bonus fix `/fitment`: `/api/vehicles` nu mai pune în cache CDN răspunsurile goale (o listă goală dinaintea importului putea rămâne „blocată" până la 1h) + banner explicativ în selector când baza de vehicule e nepopulată. — igienizare frontend + cache
- 2026-07-06 — **CATALOG V2 — preț B2B + worker (Faza 3 schelet)**: mapper-ul v2 acceptă `offer:price_b2b` (coloană sau formulă `{col}*…`), pipeline-ul îl pune în `offer_payload`, migrația `019_publish_price_b2b.sql` îl scrie în `supplier_offers.price_b2b` (recompute-ul îl denormalizează pe variantă; `/api/cart` + `/api/orders` îl folosesc deja pentru clienți B2B). Câmp „Preț B2B" în wizard. `scripts/import-worker.ts` + `npm run worker` — consumator de coadă cu `claim_next_import_job()` (SKIP LOCKED), rulează același pipeline în afara Next.js; deployabil pe Railway/Fly/VPS când feed-urile depășesc limita de 300s. — B2B end-to-end în v2 + fundația worker
- 2026-07-06 — **CATALOG V2 — Faza 5 (storefront + checkout variante)**: migrație `018_cart_orders_v2_variants.sql` (cart/order_items.variant_id, aditiv). Pagini noi: `/c` (index categorii), `/c/[slug]` (catalog generic cu fațete din `category_filter_definitions` + attrs JSONB, filtru brand, sort, paginare), `/p/[slug]` (pagina familiei cu tabel variante + add-to-cart per variantă). `/api/cart` acceptă `variant_id` și normalizează articolele v2 la forma v1 (pagina /cos neschimbată); `/api/orders` procesează variante (stoc/preț/B2B/snapshot). Fluxul complet furnizor → Import V2 → catalog → coș → comandă funcționează pentru catalogul universal. Notă perf: fațetele MVP agregă în server component (cap 500 familii) — RPC fațetat la scală. — storefront v2
- 2026-07-06 — **FIXED H3**: `public/robots.txt` adăugat (Allow /, Disallow /admin, /api/, /cos, /checkout, /cont, /auth/, /setup; link sitemap). — SEO
- 2026-07-06 — **Fitment public — „Ce se potrivește pe mașina ta?"**: pagină nouă `/fitment` cu selector cascadă Marcă→Model→An→Trim (`/api/vehicles`, cache 1h), statistici agregate din setup-urile reale (dimensiuni jante populare cu contor, anvelope populare, % fără frecări, % staggered), tabel cu primele 30 setup-uri și CTA „Jante pe X″" spre catalogul filtrat. Link „Ce se potrivește?" în Navbar (desktop + mobil). Necesită fitmenturile importate din `/admin/vehicule`. — prima features storefront pe datele de fitment
- 2026-07-06 — **CATALOG V2 — Faza 4 (UI admin import v2)**: hub `/admin/import-v2` (profiluri + istoric joburi), wizard 5 pași `/admin/import-v2/profiles/nou` cu preview live client-side (rulează `mapRow()` din motorul real pe rândurile de probă), raport job `/admin/import-v2/jobs/[id]` cu diff staging + erori grupate + acțiuni Publică/Renunță/Rollback/„Rulează importul real". Rute noi: `context` (categorii+atribute+branduri+cursuri), `sample` (upload multipart → snapshot re-folosibil SAU fetch feed cu auth). Pipeline: auto-detect format din extensie + fallback delimitator CSV. `lib/import/detect.ts` + 4 teste (38 total). Link „Import V2" în sidebar. — fluxul complet furnizor→catalog fără cunoștințe tehnice
- 2026-07-06 — **Import fitmenturi din admin (fără CLI)**: pagină nouă `/admin/vehicule` (statistici + butoane „Verifică (dry-run)" / „Importă fitmenturile"), rută `POST /api/admin/fitment-import` care rulează importul server-side din `data/fitmentgallery.csv.gz` (inclus în bundle prin `outputFileTracingIncludes`). Logica extrasă în `lib/import/fitmentGallery.ts`, partajată cu scriptul CLI (rămas pentru rulări locale/CI). Link „Vehicule" în sidebar admin. De acum lucrul se face direct pe `main`. — UX admin pentru non-developeri
