...
- 2026-06-18 — **Adăugat furnizor SIXNINE Wheels**: migrație `012_seed_sixnine_supplier.sql` (supplier id=8, slug `sixnine`, format CSV `;`, full `field_mappings`), CSV static `pimpit-web/public/feeds/sixnine.csv` (208 produse, 5 modele EVO-ONE/FF-ONE/FF-TWO/FF-THREE/OEM+ONE, stock pre-completat 99 — coloana era goală în feed-ul original). `pricing_rules` cu toate multiplicatoarele = 1 (pass-through SRP→preț final RON, per cerere). `importRunner.fetchFeed` rezolvă URL-uri relative `/...` față de `NEXT_PUBLIC_SITE_URL` (fallback `http://localhost:3000`) ca să servească feed-uri din `/public`. Multi-PCD jante (ex. `5x100, 5x114.3`) gestionat de `normalizeAndJoinPcds` existent — customer alege pattern-ul în cart via `selected_pcd` din migrația 009. Import primul rulat manual din `/admin/sincronizari`. — furnizor nou + pattern feed self-hosted
