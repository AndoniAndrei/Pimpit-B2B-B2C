-- ============================================================================
-- 017 — PUBLISH / ROLLBACK pentru import engine v2
-- publish_import_job(job_id)  — aplică atomic staging-ul în catalogul canonic
-- rollback_import_job(job_id) — restaurează ofertele la starea `previous`
-- Vezi docs/CATALOG_V2_PLAN.md §3.4
-- ============================================================================
...
CREATE OR REPLACE FUNCTION rollback_import_job(p_job_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF job.status <> 'published' THEN
    RAISE EXCEPTION 'Jobul % are status %, doar "published" se poate anula', p_job_id, job.status;
  END IF;

  FOR r IN
    SELECT * FROM import_staged_variants
    WHERE job_id = p_job_id AND published_at IS NOT NULL
    ORDER BY id DESC
  LOOP
    -- Rezolvă varianta: match direct sau după (brand, part_number) pentru create
    v_variant := r.matched_variant_id;
    IF v_variant IS NULL THEN
      SELECT v.id INTO v_variant
      FROM product_variants v
      JOIN brands b ON b.id = v.brand_id
      WHERE v.part_number = r.variant_payload->>'part_number'
        AND (
          b.id = NULLIF(r.product_payload->>'brand_id', '')::BIGINT
          OR b.slug = slugify(COALESCE(r.product_payload->>'brand_name', ''))
        )
      LIMIT 1;
    END IF;
    IF v_variant IS NULL THEN CONTINUE; END IF;

    IF r.previous IS NOT NULL THEN
      -- Restaurează starea anterioară a ofertei
      UPDATE supplier_offers
      SET raw_price      = NULLIF(r.previous->>'raw_price', '')::NUMERIC,
          raw_currency   = COALESCE(NULLIF(r.previous->>'raw_currency', ''), raw_currency),
          price          = NULLIF(r.previous->>'price', '')::NUMERIC,
          price_b2b      = NULLIF(r.previous->>'price_b2b', '')::NUMERIC,
          stock          = COALESCE((r.previous->>'stock')::INT, stock),
          stock_incoming = COALESCE((r.previous->>'stock_incoming')::INT, stock_incoming),
          is_available   = COALESCE((r.previous->>'is_available')::BOOLEAN, is_available),
          import_job_id  = NULL
      WHERE variant_id = v_variant AND supplier_id = job.supplier_id;
      n_restored := n_restored + 1;
    ELSE
      -- Oferta a fost creată de acest job → se șterge (trigger-ul recalculează
      -- prețul variantei; variantele rămase fără oferte devin inactive)
  ...
  RETURN jsonb_build_object('offers_restored', n_restored, 'offers_deleted', n_deleted);
END;
$$;

COMMENT ON FUNCTION rollback_import_job(UUID) IS
  'Anulează un job publicat: restaurează ofertele la snapshotul `previous` din '
  'staging și șterge ofertele create de job. Limitare cunoscută: modificările '
  'de atribute/media nu sunt reversate (doar ofertele) — v. CATALOG_V2_PLAN §4.';
